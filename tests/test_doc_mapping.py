...
def test_doc_document_mapping():
    ...
    if not os.path.exists(doc_path):
        print(f"错误: 找不到DOC文档 {doc_path}")
        # 尝试在上层目录查找
        parent_doc_path = os.path.join(current_dir, '../tests/毛文静-最终论文.docx')
        if os.path.exists(parent_doc_path):
            doc_path = parent_doc_path
        else:
            parent_doc_path2 = os.path.join(current_dir, '../../毛文静-最终论文.docx')
            if os.path.exists(parent_doc_path2):
                doc_path = parent_doc_path2
            else:
                print(f"错误: 找不到DOC文档 {doc_path}")
                return

    print("=== 使用DOC文档测试引用映射功能 ===")
    print(f"DOC文档路径: {doc_path}")
    ...
    print(f"\n从DOC提取到 {len(document.citations)} 个引用和 {len(document.references)} 个参考文献条目")
    ...
    # 保存详细报告
    report_path = os.path.join(current_dir, "doc_citation_mapping_detailed_report.json")
    ...
    print(f"\nDOC文档详细报告已保存到 {report_path}")


if __name__ == "__main__":
    test_doc_document_mapping()