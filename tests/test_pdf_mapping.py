...
def test_pdf_document_mapping():
    """使用PDF文档测试引用映射功能"""

    # 使用PDF文档 - 首先尝试在tests目录中查找
    pdf_path = os.path.join(current_dir, '蔡安平-Few-shot_Learning_Framework_Based_on_Adaptive_Subspace_for_Skin_Disease_Classification-1.pdf')

    # 如果在tests目录中不存在，则尝试在项目根目录查找
    if not os.path.exists(pdf_path):
        pdf_path = os.path.join(project_root, '蔡安平-Few-shot_Learning_Framework_Based_on_Adaptive_Subspace_for_Skin_Disease_Classification-1.pdf')

    # 检查文档是否存在
    if not os.path.exists(pdf_path):
        print(f"错误: 找不到PDF文档 {pdf_path}")
        print("可用的PDF文件:")
        import glob
        pdf_files = glob.glob(os.path.join(project_root, "**/*.pdf"), recursive=True)
        for pdf_file in pdf_files:
            print(f"  - {pdf_file}")
        return

    print("=== 使用PDF文档测试引用映射功能 ===")
    ...
    print(f"\n从PDF提取到 {len(document.citations)} 个引用和 {len(document.references)} 个参考文献条目")
    ...
    # 保存详细报告
    report_path = os.path.join(current_dir, "pdf_citation_mapping_detailed_report.json")
    ...
    print(f"\nPDF文档详细报告已保存到 {report_path}")


if __name__ == "__main__":
    test_pdf_document_mapping()